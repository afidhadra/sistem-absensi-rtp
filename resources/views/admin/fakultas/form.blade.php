...
<h1 class="mb-6 text-2xl font-bold text-gray-800">{{ isset($item) ? 'Edit Fakultas' : 'Tambah Fakultas' }}</h1>
...
<form method="POST" action="{{ isset($item) ? route('admin.fakultas.update', $item) : route('admin.fakultas.store') }}">
    ...
    <div class="flex gap-2">
        ...
        <a href="{{ route('admin.fakultas.index') }}" class="rounded-lg bg-gray-200 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-300">Batal</a>
    </div>
</form>
@endsection
