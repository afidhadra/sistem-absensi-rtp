...
<div class="overflow-hidden rounded-xl bg-base-100 shadow">
    <table class="table table-zebra table-sm">
        <thead>
            <tr class="text-xs uppercase text-base-content/50"><th>Hari</th><th>Jam</th><th>Ruangan</th><th>Dosen</th><th>Matkul</th><th>Kelas</th><th class="text-right">Aksi</th></tr>
        </thead>
        <tbody>
            @forelse ($items as $item)
                <tr class="hover">
                    ...
                    <td class="text-right whitespace-nowrap">
                        ...
                        <form method="POST" action="{{ route('admin.jadwal.destroy', $item) }}" class="inline" x-data @submit.prevent="$store.confirm.ask($event, 'Hapus data ini?', 'Hapus')">
                            @csrf @method('DELETE')
